...
import '../../home/models/dashboard_metrics.dart';
...
class FirebaseRepository {
  ...
  Stream<List<Project>> watchProjects({String? memberId}) {
    ...
    return query.snapshots().map((snapshot) {
      final projects =
          snapshot.docs.map(Project.fromFirestore).toList();
  ...
  Stream<List<PersonRecord>> watchPeople() => _firestore
      ...
      .map(
        (snapshot) => snapshot.docs
            .map(PersonRecord.fromFirestore)
            .toList(),
      );
  ...
  Future<UserProfile?> getProfile(String uid) async {
    ...
    return document.exists
        ? UserProfile.fromFirestore(document)
        : null;
  }
  ...
  Future<String> uploadProfileImage({
    ...
    final reference =
        _storage.ref('profile_images/$uid.$extension');
  ...
  Stream<DashboardMetrics?> watchDashboardMetrics(
    ...
    return _firestore
      ...
      return DashboardMetrics.fromFirestore(
        snapshot.data()!,
      );
    });
  }

  Stream<List<PortalBjIndicator>> watchPortalBjIndicators(
    int year,
  ) {
    return _firestore
        .collection('portal_bj_indicators')
        .where(
          'year',
          isEqualTo: year,
        )
        .snapshots()
        .map((snapshot) {
      final indicators = snapshot.docs.map((doc) {
        return PortalBjIndicator.fromFirestore(
          doc.id,
          doc.data(),
        );
      }).toList();
...
class Hierarchy {
  ...
  static const awaitingRoleAssignment =
  ...
  static bool canManagePeople(String role) =>
      level(role) >= level(management);

  static bool canViewProjects(String role) =>
      level(role) >= level(member);

  static bool canManageProjects(String role) =>
      level(role) >= level(management);

  static bool canViewFinance(String role) =>
      level(role) >= level(directory);

  static bool canManageAll(String role) =>
      level(role) >= level(administrator);

  static bool canAssignRole(String role) =>
      role.trim() == presidency ||
      role.trim() == vicePresidency;
